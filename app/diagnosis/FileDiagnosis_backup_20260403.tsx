...
export default function FileDiagnosis({C}: {C: any}) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 双方向確認チャット */}
      {(phase==="clarifying"||phase==="diagnosing"||phase==="done") && (
              ...
              {!isReady ? (
                <div style={{display:"flex",gap:"8px"}}>
                  <input value={userInput} onChange={e=>setUserInput(e.target.value)}
                    ...
                    placeholder="AIの質問に回答 または AIへの質問を入力（Enterで送信）"
                  ...
                  <button onClick={handleChatSend} disabled={!userInput.trim()||chatLoading}
                    style={{background:(!userInput.trim()||chatLoading)?"rgba(255,255,255,0.05)":"linear-gradient(135deg,#6366f1,#8b5cf6)",borderRadius:"10px",padding:"10px 20px",color:"white",fontWeight:700,fontSize:"13px",cursor:(!userInput.trim()||chatLoading)?"not-allowed":"pointer",border:"none",whiteSpace:"nowrap" as const}}>
                    送信
                  </button>
                </div>
              ) : (
      ...
      {result && phase==="done" && (
        <div className="space-y-4">
          {result.overview&&<div style={{...S.card}}><p style={{color:"rgba(255,255,255,0.4)",fontSize:"10px",fontWeight:700,letterSpacing:"0.15em",marginBottom:"8px"}}>OVERVIEW</p><div style={{color:"rgba(255,255,255,0.85)",fontSize:"13px",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:renderMd(toStr(result.overview))}}/></div>}
          {result.structure&&<div style={{background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:"14px",padding:"16px 20px"}}><p style={{color:"#a5b4fc",fontSize:"10px",fontWeight:700,letterSpacing:"0.15em",marginBottom:"8px"}}>🏗️ 構造診断</p><div style={{color:"rgba(255,255,255,0.8)",fontSize:"13px",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:renderMd(toStr(result.structure))}}/></div>}
          {result.issues&&<div style={{background:"rgba(234,179,8,0.08)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:"14px",padding:"16px 20px"}}><p style={{color:"#fde047",fontSize:"10px",fontWeight:700,letterSpacing:"0.15em",marginBottom:"8px"}}>🎯 課題仮説</p><div style={{color:"rgba(255,255,255,0.8)",fontSize:"13px",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:renderMd(toStr(result.issues))}}/></div>}
          {result.action_plan&&<div style={{background:"rgba(16,185,129,0.08)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:"14px",padding:"16px 20px"}}><p style={{color:"#6ee7b7",fontSize:"10px",fontWeight:700,letterSpacing:"0.15em",marginBottom:"8px"}}>📋 実行計画</p><div style={{color:"rgba(255,255,255,0.8)",fontSize:"13px",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:renderMd(toStr(result.action_plan))}}/></div>}
          {result.key_metrics&&<div style={{background:"rgba(139,92,246,0.08)",border:"1px solid rgba(139,92,246,0.2)",borderRadius:"14px",padding:"16px 20px"}}><p style={{color:"#c4b5fd",fontSize:"10px",fontWeight:700,letterSpacing:"0.15em",marginBottom:"8px"}}>📊 重要指標</p><div style={{color:"rgba(255,255,255,0.8)",fontSize:"13px",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:renderMd(toStr(result.key_metrics))}}/></div>}
          {result.risks&&<div style={{background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:"14px",padding:"16px 20px"}}><p style={{color:"#fca5a5",fontSize:"10px",fontWeight:700,letterSpacing:"0.15em",marginBottom:"8px"}}>⚠️ リスク</p><div style={{color:"rgba(255,255,255,0.8)",fontSize:"13px",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:renderMd(toStr(result.risks))}}/></div>}
        ...
        <div style={{...S.card,padding:"14px 18px"}}>
          ...
          <div className="space-y-2">
            {savedResults.map((r,i)=>(
              <div key={i} onClick={()=>{setResult(r.data);setPhase("done");}}
